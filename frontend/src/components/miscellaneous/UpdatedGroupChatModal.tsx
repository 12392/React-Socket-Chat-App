...
import { Chat, GroupChat } from "../../model/chats";
...
const UpdatedGroupChatModal = ({ fetchAgain, setFetchAgain }) => {
  ...
  const handleAddUser = async (user1: User) => {
    ...
    if ((selectedChat as GroupChat).groupAdmin._id !== user?._id) {
      toast({
  ...
  const handleRemove = async (user1: User) => {
    if (
      (selectedChat as GroupChat).groupAdmin._id !== user?._id &&
      user1._id !== user?._id
    ) {
      toast({
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{(selectedChat as Chat).chatName}</ModalHeader>
          <ModalCloseButton />
